...
v0.9.1 支持单组织多人受控部署，并增加 GitHub Actions 最小 CI 门禁。生产环境必须配置固定的 `GEO_INTERNAL_API_KEY`、`GEO_BOOTSTRAP_OWNER_PASSWORD`，并在公网暴露前增加 HTTPS、反向代理、VPN、IP allowlist 或其他外部访问控制。
...
v0.9.1 不是完整 SaaS：暂不包含多租户隔离、OAuth/SSO、MFA、真实支付计费、生产数据库迁移、真实第三方发布凭据或邮件邀请流程。
