...
Status: v0.16.0 one-organization team-access workspace with built-in login, RBAC, launch preflight, International GEO site audit, guarded live crawl evidence, evidence-backed scoring, GEO asset generation, AI visibility measurement foundation, evidence-driven local asset opportunities, local-rule article generation, multi-platform rewrite generation, high-authority publishing platform list, review-only package queue, manual tracking records, controlled deployment docs, and minimal GitHub CI.
...
v0.16.0 支持单组织多人受控部署，并在 International GEO 中提供规则优先的站点 GEO 审计、受保护的 live crawl evidence、证据化 100 分评分拆解、`llms.txt` / JSON-LD / FAQ / 内容分发资产生成、AI visibility measurement foundation、evidence-driven asset opportunities、local generation queue、generated local previews、approve/reject review state、local-rule 完整文章草稿生成、多平台改写稿生成、生成记录，以及高权重发布平台清单、发布包队列和手动收录/推荐追踪。生产环境必须配置固定的 `GEO_INTERNAL_API_KEY`、`GEO_BOOTSTRAP_OWNER_PASSWORD`，并在公网暴露前增加 HTTPS、反向代理、VPN、IP allowlist 或其他外部访问控制。
...
v0.16.0 不是完整 SaaS，也不是实时 AI 引擎监控平台或自动发布系统：暂不包含多租户隔离、OAuth/SSO、MFA、真实支付计费、生产数据库迁移、真实第三方发布凭据、邮件邀请流程、ChatGPT / Gemini / Claude / Perplexity / Google AI Overviews / Copilot / Bing 查询、真实 SERP 采集、实时索引验证或自动第三方发布。当前 live crawl 仅限受保护地抓取提交站点的 homepage、`robots.txt`、`sitemap.xml` 和 `/llms.txt` 证据；证据化评分只基于本地输入与这些抓取证据，不代表真实 AI 引擎收录或推荐排名。v0.13 visibility runs 默认只生成 `unavailable` snapshots；只有未来接入并批准的 provider evidence 才能标记为 `measured`，演示或种子数据必须标记为 `simulated`。v0.14 evidence assets 只生成可审阅的本地资产、队列项和 approve/reject 状态；v0.15 publishing workflow 只做本地高权重平台参考、planning/handoff、review-only package queue 和 manual/local tracking；v0.16 content generation workflow 使用 `local_rules` 从已审核证据资产生成完整文章草稿，并从已审核文章生成多平台改写稿。它不外部发布、不保存外部平台凭据、不调用外部 LLM，不代表真实索引、AI mention、引用、推荐或外部分发；平台说明只表示发布到这些公开高可见度渠道可能增加被 AI 检索、引用和推荐的概率。
