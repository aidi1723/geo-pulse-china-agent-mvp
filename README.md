...
Status: v0.11.0 one-organization team-access workspace with built-in login, RBAC, launch preflight, International GEO site audit, guarded live crawl evidence, GEO asset generation, controlled deployment docs, and minimal GitHub CI.
...
v0.11.0 支持单组织多人受控部署，并在 International GEO 中提供规则优先的站点 GEO 审计、受保护的 live crawl evidence、`llms.txt` / JSON-LD / FAQ / 内容分发资产生成。生产环境必须配置固定的 `GEO_INTERNAL_API_KEY`、`GEO_BOOTSTRAP_OWNER_PASSWORD`，并在公网暴露前增加 HTTPS、反向代理、VPN、IP allowlist 或其他外部访问控制。
...
v0.11.0 不是完整 SaaS，也不是实时 AI 引擎监控平台：暂不包含多租户隔离、OAuth/SSO、MFA、真实支付计费、生产数据库迁移、真实第三方发布凭据、邮件邀请流程、ChatGPT / Gemini / Claude / Perplexity / Google AI Overviews / Copilot 查询、真实 SERP 采集或自动第三方发布。当前 live crawl 仅限受保护地抓取提交站点的 homepage、`robots.txt`、`sitemap.xml` 和 `/llms.txt` 证据。
