...
This guide covers the v0.10.0 one-organization team-access deployment profile for GEO Pulse.

For stage-level scope and closing language, see [v0.9 Stage Closeout](STAGE_V0_9_CLOSEOUT.md), [v0.9.1 Stage Closeout](STAGE_V0_9_1_CLOSEOUT.md), and [v0.10 Stage Closeout](STAGE_V0_10_CLOSEOUT.md).
...
- International GEO site audit input, durable rule-first audit records, check-level recommendations, and generated GEO asset previews.
- Health checks.
...
- Live crawling of submitted websites, real SERP collection, measured AI engine inclusion checks, or automatic third-party publishing.
...
The v0.10.0 deployment stores state in a local JSON file and provides built-in local backup controls in Settings -> Brand Knowledge -> Runtime and Data.
...
```bash
docker build -t geo-pulse:v0.10.0 .
```

## Stage Closeout Language

GEO Pulse v0.10.0 is ready for controlled one-organization team deployment. It includes built-in login, role-based access, local workflows, International GEO site audit and asset generation, connector configuration, connector testing, connector diagnostics, local backup import/restore, launch preflight, production startup guardrails, health checks, GEO/SEO static files, Docker packaging, minimal GitHub CI, and documentation for rollback. It must still be protected by an external access layer and should not be presented as a complete SaaS platform or real-time AI search monitoring platform until durable database storage, OAuth/SSO, MFA, real integrations, monitoring, live crawl evidence, measured AI visibility data, and multi-tenant controls are implemented.
