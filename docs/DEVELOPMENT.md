...
GEO Pulse China Agent v0.17.0 is intentionally small:
...
- UI rendering for the prototype pages, including International GEO site audit, crawl evidence, score breakdown, visibility panels, prompt snapshots, `导入测量证据`, evidence asset opportunities, queue state, review state, asset previews, article generation queue, platform rewrite queue, generation runs, high-authority publishing platform list, package queue, and manual tracking panels.
- Persistence, scheduler, audit, connector, source adapter, campaign, publishing, visibility, and International GEO site audit/crawl evidence/visibility/evidence-import/evidence-asset/content-generation/publishing flows.
...
- Visibility measurement changes: document `measured`, `simulated`, and `unavailable` semantics, manual human-entered evidence requirements, future provider evidence requirements, and any no-real-provider boundary in the same change.
